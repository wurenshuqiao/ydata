...
/**
 * This is a helper class designed to work with byte conversionת�� and representation of numbers
 * 
...
class HessianParser extends HessianProtocolHandler{
	...
	function readString(){
		...
		while(!$end) {
			...
			// Some UTF8 characters are represented ��ʾwith more than one byte to we need
			// to read every character to find out if we need to read in advanceԤ��.
			for($i=0;$i<$len;$i++){
				$ch = $this->read(1);
				$charCode = ord($ch);
				if($charCode < 0x80)
...
class HessianWriter extends HessianProtocolHandler{
	...
	function writeStringData($value){
            // pack包装数据变成二进制字符串
            // strlen得到 字符串的字节数，而长度是unicode的字符数
		//$this->stream .= pack('n',strlen($value));
		//$this->stream .= utf8_encode($value);
            // 修改
            $this->stream .= pack('n',  HessianWriter::utf8_strlen($value));
            $this->stream .= $value;
        ...
        /*
         * 参考 http://robinson.javaeye.com/blog/778979
         * utf8_strlen函数返回的是这个utf8编码的字符串的真正字符数
         */
