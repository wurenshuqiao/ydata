...
/**
 * <BLOCKQUOTE>
 * Proxy that allows RPC method calls to a remote Hessian compatible相容的 web-service
 ...
 * natively. If your PHP installation doesn't support overload, you can use direct直接 calls to the __call() 
 * method using the arguments参数 as detailed in <A HREF="http://us3.php.net/manual/en/ref.overload.php">http://us3.php.net/manual/en/ref.overload.php</A> 
 ...
 * </code>
 * <BR>
 * <B>WARNING:</B> As in PHP 4.3.x, all method name metadata (get_class_methods(), overload extension __call()) is internally内部地 
 * lower-cased so if you call a camel骆驼 case remote method name, like 'getUsers', the call will result导致 into a no such
 * method fault because PHP will interpret解释 the method name as 'getusers'. This is especially特别 bad when calling Java
 * services where many method names are camel cased. Sad but true.<BR>
 * <BR>
 * To counterfeit伪造 this, use {@link HessianPHP.Hessian#remoteMethod remoteMethod()}  in the proxy object to tell Hessian how to call the method with the 
